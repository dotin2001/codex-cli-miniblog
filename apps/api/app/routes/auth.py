...
from app.auth import (
    ...
    current_user_from_authorization_header,
    current_user_from_refresh_cookie,
    public_user,
)
...
def login():
    ...
    response = jsonify(
        {"accessToken": create_access_token(user), "user": public_user(user)}
    )
...
def refresh():
    user = current_user_from_refresh_cookie()
    if user is None:
        return _refresh_token_error()

    return jsonify({"accessToken": create_access_token(user)}), 200
