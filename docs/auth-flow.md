...
`GET /auth/me` returns the public user object for a valid bearer access token. `POST /blogs`, `PATCH /blogs/<slug>`, `DELETE /blogs/<slug>`, and `POST /blogs/<slug>/comments` use the same bearer-token validation for authenticated writes. Missing, malformed, invalid, expired, or unknown-user tokens return:
